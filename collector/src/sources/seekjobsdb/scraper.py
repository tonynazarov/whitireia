...
class Scraper(BaseScraper):
    __total_jobs_id: dict = {'data-automation': 'searchResultBar'}
    __total_jobs_el: str = 'span'
    __result_container = 'jobListing'
    __index: str = '/search-jobs/{query}/{page}'

    def parse_view_page(self, url: str) -> Page:
        url = self._get_path(url).replace('th/th', 'th').replace('hk/hk', 'hk')
    ...
    def get_jobs_total(self, html: str) -> str:
        ...
        jobs_total = re.findall('f (\d+) j', jobs_total)[0]
